...
2. Download "Basic Package" — e.g., instantclient-basic-windows.x64-21.9.0.0.0dbru.zip
...
If you get an error like "DPI-1047: Cannot locate a 64-bit Oracle Client library":
→ Make sure Oracle Instant Client is extracted and PATH is set (Step 3.2)
...
The server already serves the frontend. Visit:
```
http://localhost:3000
```
...
✅ CREATE — Click any "+ Add" button, fill the form, submit
✅ READ   — All tables load automatically; use the search box to filter
✅ UPDATE — Click "Edit" on any row, change values, Save
✅ DELETE — Click "Delete" on any row, confirm

Tables to demo for evaluator:
1. Passengers (with Business/Economy/VIP subtypes)
2. Flights (status changes)
3. Crew (Pilot/CabinCrew subtypes)
4. Boarding Passes
5. Security Checks
6. Baggage
